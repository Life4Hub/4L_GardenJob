...
function ManageJob()
    ...
    ESX.TriggerServerCallback('rlo_gardening:callback:checkJobStatistics', function(experience)
        ...
        jobLevel = math.floor(jobExperience / 100)

        local startedJobMenuElements = {
            {label = '❇️ Check Experience and Level', name = 'job_level'},
            {label = '🪙 Job beenden('..collectedPoints..' Punkte)', name = 'redeem_points'},
            {label = '❌ Job beenden ohne Punkte', name = 'forced_end'},
        }
        ...
        if activeJob then
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gardening_manage_menu', {
                ...
                align = 'top-left',
                elements = startedJobMenuElements
            }, function(data, menu)
                ...
                elseif data.current.name == 'forced_end' then
                    menu.close()

                    local forcedEndMenu = {
                        {label = 'Ja, Job OHNE Punkte beenden', name = 'forced_end'},
                        {label = 'Nein, fortfahren', name = 'cancel'},
                    }
                    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gardening_manage_menu', {
                        css = 'bossmenu',
                        title = 'Gardener | Current Level: '..tostring(jobLevel),
                        align = 'top-left',
                        elements = forcedEndMenu
                    }, function(data, menu)
                        if data.current.name == 'forced_end' then
                            menu.close()
                            EndJob()
